...
//+++++Axes Style++++++++++
  ...
  //[5]=color
	 ...
	 strcpy_s(m_Cdf.m_sTitles[0],"Result Chart");//The Dialog Title;
	 strcpy_s(m_Cdf.m_sTitles[1],"Experimental Results");//The Figure Title;
     strcpy_s(m_Cdf.m_sTitles[2],"X");//The X Title
	 strcpy_s(m_Cdf.m_sTitles[3],"Y");//The Y Title
     if(m_Dimension==1)
	 {
...
bool CDrawCurve::DrawCurve(HDC hdc,int Left,int LeftE,int Right,int RightE,int Up,int UpE,int Bottom,int BottomE)
{
   ...
   if(m_Cdf.m_bAxes[6]>0)
   {
     ...
     for(i=0;i<=m_Cdf.m_bAxes[3];i++)
	 {
	   sprintf_s(tmp,"%.2f",displayx[i]*times);
	   temp=tmp;
	   if(cases==0)
	   {
     ...
     for(i=0;i<=m_Cdf.m_bAxes[4];i++)
	 {
	   ...
	   sprintf_s(tmp,"%.2f",displayy[i]*times);
	   temp=tmp;
	   ::TextOut(hdc,posix,posiy,temp,temp.GetLength());
	 }
   }

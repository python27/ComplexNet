...
void CDialogDrawCurve::View2Data()
{
   ...
   strcpy_s(cdf.m_sTitles[0],m_dlgtitle);
   strcpy_s(cdf.m_sTitles[1],m_sfigtitle);
   strcpy_s(cdf.m_sTitles[2],m_sxtitle);
   strcpy_s(cdf.m_sTitles[3],m_sytitle);
   c.SetCDFData(cdf);
   c.CheckCDFData();
   c.GetCDFData(&cdf);
}
...
void CDialogDrawCurve::OnPaint() 
{
	...
	sprintf_s(str," and the length of data is %ld",m_lNumberofSamples);
	tem=tem+str;
